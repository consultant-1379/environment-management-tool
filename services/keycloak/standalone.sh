...
sed -i -e "s/HOSTNAME_PLACEHOLDER/${HOST_NAME}/g" /opt/jboss/keycloak-server-config.json
if [[ "${development_or_production}" == "DEVELOPMENT" ]]; then
    sed -i -e "s/THUNDERBEEFUNCTIONALUSERPASSWORD/${TB_FUNCTIONAL_USER_PASSWORD}/g" /opt/jboss/keycloak-server-config.json
    SERVER_OPTS="-Dkeycloak.migration.action=import -Dkeycloak.migration.provider=singleFile -Dkeycloak.migration.file=/opt/jboss/keycloak-server-config.json -Dkeycloak.migration.strategy=OVERWRITE_EXISTING"
else
    SERVER_OPTS=""
fi
...
while true; do
   if [ "x$LAUNCH_JBOSS_IN_BACKGROUND" = "x" ]; then
      ...
      # If we're not on production/staging, we won't have SSL certs and so we must set sslRequired to none
      if [[ "${development_or_production}" == "DEVELOPMENT" ]]; then
